...
server.on('message', (msg, rinfo) => {
  ...
  if (8000<rinfo.port&& rinfo.port<9000){
    ...
    console.log('app',i);
    console.log(`app_id = ${i.id} key = ${i.key} value = ${i.value}`);
    ...
    console.log(j);
    if (i.id == 'app' && i.interval == 'interval'){ //interval이 오면 interval을 다시 보내줌 
      server.send(j,8001,HOST,(err,bytes)=>{
        if (err) throw err;
    ...
    if (i.id == 'app2' && i.interval == 'interval'){ //interval이 오면 interval을 다시 보내줌 
      server.send(j,8002,HOST,(err,bytes)=>{
        if (err) throw err;
        console.log('interval');

      })
    }
    if (i.id == 'app3' && i.interval == 'interval'){ //interval이 오면 interval을 다시 보내줌 
      server.send(j,8003,HOST,(err,bytes)=>{
        if (err) throw err;
        console.log('interval');

      })
    }
    else if(i.id == 'app' && i.interval != 'interval' && i.commit !='commit') {
    ...
    else if(i.id == 'app2' && i.interval != 'interval' && i.commit !='commit') {
      server.send(j, 9001, HOST, function(err, bytes) {
        if (err) throw err;
        console.log('Message Handler sent to ' + HOST +':'+ 9001);
    ...
    else if(i.id == 'app3' && i.interval != 'interval' && i.commit !='commit') {
      server.send(j, 9001, HOST, function(err, bytes) {
        if (err) throw err;
        console.log('Message Handler sent to ' + HOST +':'+ 9001);
