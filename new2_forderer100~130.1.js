...
fs.writeFile('./ledger1.txt','start\n');
...
var before_state = 'follower'; // favorite이candidate였다가 다른  candidate가 생겼을때 다시 favorite으로
//돌아가야하므로 체크하는 변수


var before_state_check_leader = 'follower';//10의배수를 처음 받았는지 아닌지 확인하는 변수 


const _sleep = (delay) => new Promise((resolve) => setTimeout(resolve, delay));

var orderer1 = '{"id":"orderer1","state":"follower","term":1} '; //JSON형식으로 데이터 저장 
if (orderer1.state == 'leader'){
  random1 = 100
} else{
  random1 = 110
}

var orderer_parse = JSON.parse(orderer1);
...
client.bind({
  ...
  const check=(value,state,array,rejoin,wait)=>{
    if(value%1000==0 && state=='leader'){
      ...
      if(array==0){
        dead_array = fs1.readFileSync('ledger1.txt').toString().split("\n");
...
client.on("listening", function () {
  ...
  console.log("orderer1 listening " +
...
client.on('message', (msg, rinfo) => {
  console.log(`orderer1 got: ${msg} from ${rinfo.address}:${rinfo.port}`);
  ...
  if(orderer_parse.state=='rejoin' && i.rejoincopy=='send'){ //오더러의 상태가 rejoin이고 카피 메시지를 리더가 보냈으면
    ...
    fs.appendFile('./ledger1.txt',copy_ledger)
        .then(()=>{
            return fs.readFile('./ledger1.txt');
        })
        .then ((data)=>{ //동기로 사용하기 위해 ()함수 앞에 async를 붙ㅌ여주고
            console.log('ledger1:',data.toString());
 ...
 /* if(i.rejoin_state == 'rejoin'){
  ...
  if(orderer_parse.rejoin == 'yes' &&i.copy!='ok' &&i.copy !='finish'){ 
    ...
    "leaderport":9001,"rejoincopy":"send"}`;
  ...
  if (i.id==='app'&& orderer_parse.rejoin !='yes' &&i.finish !='finish'){ 
    ...
    if(before_cnt != i.cnt){  //이전 카운트와 현재 들어온 메시지의 카운트가 다를경우만 장부에 저장
      ...
      if(orderer_parse.state =='dead'){ //애플리케이션의 값을 5개 받으면 
        ...
        ,"rejoin_state":"${orderer_parse.state}","port":9001,"rejoinstart":${start}}`;
        ...
        fs.appendFile('./ledger1.txt',ledger)
        .then(()=>{
            return fs.readFile('./ledger1.txt');
        })
    ...
    if (orderer_parse.state == 'leader' && orderer_parse.rejoin !='yes' &&i.finish !='finish'){
      ...
      var leader = {"msg":"copy","id":"orderer1","state":"leader","term":orderer_parse.term,"port":9001 } ;//leader면 카피 메시지를 전송하기 위해 json형식으로 선언//간소화 가능할듯
      //orderer1.xx = xx이런식으로 
    ...
    if (orderer_parse.state == 'candidate'){
      ...
      var leader = {"msg":"copy","id":"orderer1","state":"candiate","term":orderer_parse.term,"port":9001 } ;//leader면 카피 메시지를 전송하기 위해 json형식으로 선언//간소화 가능할듯
      //orderer1.xx = xx이런식으로 
      var copymsg = mergeJSON.merge(i,leader);//트랜잭션과 합쳐서 전송
      console.log('copymsg',copymsg);
  ...
  if (i.id==='app2'&& orderer_parse.rejoin !='yes' &&i.finish !='finish'){ 
  ...
  if (i.id==='app3'&& orderer_parse.rejoin !='yes' &&i.finish !='finish'){ 
 ...
 // console.log('cmtime :',cmtime);
  ...
  if (before == i.cnt){ // before = 0을 초기값으로 갖는 이전값과 commitmsg.cnt를 비교 해서 같으면
    ...
    if(commitmsg.commit_cnt ==2 ){
      ...
      orderer_parse.rejoin = 'dead'//텀이빠른애가 와서 바뀐경우 
      commit_cnt.commit_cnt = 0;
      //before+=1;
      var commit_ledger = `key = ${checkcommit_msg.key} value = ${checkcommit_msg.value} cnt = ${checkcommit_msg.cnt}\n`
        
      fs.copyFile('cmledger.txt','ledger1.txt')//cmledger의 장부를 먼저 복사해 오고 ledger의 값을 장부에 추가 왜냐하면 
      ...
      .then(()=>{
          ...
          fs.readFile('./ledger1.txt')
              .then((data)=>{
                  console.log(data.toString());
                    fs.appendFile('./ledger1.txt',ledger)
                      .then(()=>{
                        return fs.readFile('./ledger5.txt');
                        })
                        .then((data)=>{
                           console.log('ledger1:',data.toString());
  ...
  if((i.candidate =='orderer5'||i.candidate =='orderer2'||i.candidate =='orderer3'||i.candidate =='orderer4')
  && orderer_parse.rejoin !='yes'&&orderer_parse.state != 'dead' &&orderer_parse.state !='rejoin'&&orderer_parse.state != 'favorite'){
   ...
   if(i.candidate == 'orderer1'&& orderer_parse.state!='leader'&& orderer_parse.rejoin !='yes' &&orderer_parse.state !='rejoin'&&orderer_parse.state != 'dead'){
    ...
    if(orderer_parse.state == 'leader' && i.commit == 'commit'  && orderer_parse.rejoin !='yes')
    {
      ...
      fs.copyFile('ledger1.txt','cmledger.txt')
              .then(()=>{
                  console.log('복사완료');
    ...
    {
      if((i.fav1 =='1' || i.fav2 == '1')&&orderer_parse.state != 'candidate'){ 
      ...
      console.log('this is commit');
      fs.copyFile('cmledger.txt','ledger1.txt')
              .then(()=>{
                  console.log('cmledger 복사완료');
    ...
    if(i.leader == 'noleader'){
        const timer = async () => {
          try{
            ...
            /*client.send(vote_str,PORT, HOST, ()=>{
              console.log('no leader, sent term',vote_str)
            });*/
        }
